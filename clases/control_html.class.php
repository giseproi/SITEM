...
* maf_controles Class 
*
* Esta clase se utiliza para armar la sección de código de los diferentes controles HTML; cada uno de
...
function cuadro_texto($propiedades)
{
	...
	if(array_key_exists('etiqueta',$propiedades))
	{
		...
		// Coloca dos celdas una con la etiqueta y otra con el control
		$cadena_html="<td>".$label."\n</td>\n<td align=\"undefined\" valign=\"undefined\"><input ".$disabled." ".$readonly." ".$maxlength." ".$size." ".$tabindex." ".$name." ".$value."></td>";
		return $cadena_html;
}//Fin del método cuadro_texto
...
function cuadro_seleccion($propiedades)
{
	...
	if(array_key_exists('seleccionado',$propiedades))
	{
		...
		$checked='checked="'.$propiedades['seleccionado'].'"';
	...
	if(array_key_exists('etiqueta',$propiedades))
	{
		...
		if(array_key_exists('fondo',$propiedades))
	{
		
		$background='style="background-color: '.$propiedades['fondo'].';"';
		...
		$cadena_html="<tr>\n<td ".$background." align=\"undefined\" valign=\"undefined\">".$label."\n</td>\n<td ".$background." align=\"undefined\" valign=\"undefined\"><input ".$disabled." ".$checked." ".$tabindex." ".$name." ".$value." type=\"checkbox\">\n</td>
</tr>";
...
function boton_radial($propiedades)
{
	...
	//Verifica las propiedades especificadas y genera la cadena correspondiente:
	//En este control el grupo es tomado como el nombre del control
				...
				if(array_key_exists('fondo',$propiedades))
	{
		...
		$cadena_html="<td ".$background." align=\"undefined\" valign=\"undefined\">".$label."\n</td>\n<td ".$background." align=\"undefined\" valign=\"undefined\"><input ".$disabled." ".$checked." ".$tabindex." ".$grupo." ".$value." type=\"radio\">\n</td>";
//To Do: Implementar para poder poder el label delante o detrás del control
		return $cadena_html;
}
...
function boton_submit($propiedades)
{
	...
	if(array_key_exists('nombre',$propiedades))
	{
		...
		$cadena_html="<input ".$disabled." ".$tabindex." ".$name." ".$value." type=\"submit\">";
//To Do: Implementar para poder poder el label delante o detrás del control
		return $cadena_html;
}
...
function boton_generico($propiedades)
{
	...
	/**
	 *@method lista_seleccion
	 * @param array propiedades
	 * @return void
	 * @access public
	 * Los encabezados <select name="nombre_componente"></select> deben ser colocados por la rutina que invoca este método
	 */

function lista_seleccion($propiedades)
{
	...
	if(array_key_exists('etiqueta',$propiedades))
	{
		...
		$cadena_html="<option value=\"".$value."\">".$etiqueta."</option>";
//To Do: Implementar para poder poder el label delante o detrás del control
		return $cadena_html;
}
